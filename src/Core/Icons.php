...
final class Icons {
	...
	/**
	 * Curated icon SVGs (viewBox 0 0 24 24, stroke-based).
	 *
	...
	private static array $icons = [
		...
		'replace'         => '<path d="M14 4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v4a2 2 0 0 1-2 2h-4a2 2 0 0 1-2-2z"/><path d="M2 14a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v4a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2z"/><path d="m7 21 3.37-3.37"/><path d="m12 7 1.44-1.44"/><path d="m15.7 4.3 2 2"/><path d="M22 13v2a2 2 0 0 1-2 2h-5.37"/>',
		...
		'construction'    => '<rect x="2" y="6" width="20" height="8" rx="1"/><path d="M17 14v7"/><path d="M7 14v7"/><path d="M17 3v3"/><path d="M7 3v3"/><path d="M10 14 2.3 6.3"/><path d="M14 6l7.7 7.7"/><path d="M8 6l8 8"/>',
	 ...
	 *
	 * @param string $name  Lucide icon name (key from the $icons array).
	 * @param int    $size  Width and height in pixels.
	 * @param string $class Additional CSS class(es).
	 *
	 * @return string SVG markup or empty string if the icon doesn't exist.
	 */
	public static function render( string $name, int $size = 20, string $class = '' ): string {
		$paths = self::$icons[ $name ] ?? null;

		if ( $paths === null ) {
			return '';
		}

		$cssClass = 'mdkit-icon' . ( $class !== '' ? ' ' . esc_attr( $class ) : '' );
	...
	/**
	 * Return a base64-encoded SVG for use in `add_menu_page()`.
	 *
	 * WordPress expects `data:image/svg+xml;base64,...` for custom menu icons.
	 * Uses the MUDRAVA "M" brand mark instead of a Lucide icon.
	 */
	public static function menuIcon(): string {
		$svg = '<svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">'
			. '<path d="M1.23084e-05 0.599993V-5.96046e-06H3.68001L9.44001 12.24L8.78001 13.76H6.18001L1.23084e-05 0.599993ZM11.3 7.97999H11.22L9.88001 11.22L8.16001 7.53999L11.3 -5.96046e-06H15.1V13.76H11.3V7.97999ZM1.23084e-05 2.57999L3.68001 10.42V13.76H1.23084e-05V2.57999Z" fill="white"/>'
			. '</svg>';
	...
	/**
	 * Return a base64-encoded SVG for a specific icon name.
	 *
	 * Useful for registering standalone admin menu items with a custom icon.
	 *
	 * @param string $name Lucide icon name (key from the $icons array).
	 */
	public static function menuIconByName( string $name ): string {
		$paths = self::$icons[ $name ] ?? self::$icons['layers'] ?? '';
